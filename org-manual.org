...
** Manipulation de liens 
   ...
   + =C-c l= ou =org-store-link= :: stocke un lien à l'emplacement
        actuel. Cette commande est /globale/ (vous devez créer la
        liaison vous-même) qui peut être utilisée dans tout tampon
        pour créer un lien. Le lien sera stocké pour des insertions
        ultérieures dans un tampon Org (voir en dessous). Quel genre
        de lien sera créé dépend du tampon actuel :

	*Tampons du mode Org*

	Pour des fichiers Org, s'il y a une '=<<cible>>=' au curseur,
        le lien pointe vers une cible. Autrement il pointe vers la
        rubrique actuelle, qui sera aussi la description[fn:29].

	Si la rubrique a une propriété CUSTOM_ID, un lien vers ce
        CUSTOM ID sera stocké. De plus ou alternativement (selon la
        valeur de =org-id-link-to-org-use-id=), un unique ID global de
        propriété sera créé et/ou utilisé pour construire un
        lien[fn:30]. Donc utiliser cette commande dans des tampons Org
        créera potentiellement deux liens : un lisible depuis le
        custom ID, et un globalement unique et qui fonctionne même si
        l'entrée est déplacée dans le fichier. Ensuite, lorsqu'insérer
        le lien vous devez décider lequel utiliser.
...
[fn:29] Si la rubrique contient un horodatage, il sera effacé du lien
